  ...
  try {
    ...
    await aiService.processChatStream(
      ...
      (chunk: string) => {
        buffer += chunk;
        if (buffer.length >= 10) {
          res.write(`data: ${JSON.stringify({ sessionId, chunk: buffer, done: false })}
...
`);
        ...
        res.write(`data: ${JSON.stringify({ sessionId, chunk: '', disclaimer, done: true })}
...
`);
    ...
    if (!res.headersSent) {
      ...
      res.write(`data: ${JSON.stringify({ error: '服务内部错误，请稍后重试' })}
